...
release-bin-linux-arm64: vendor
	docker run --platform linux/arm64 --rm -v `pwd`:$(SOURCE_PATH) --env GOOS=linux --env GOARCH=arm64 $(CONTAINER_ARM) go build -mod=vendor -o $(BIN_NAME) $(CUSTOM_TAGS) $(LDFLAGS) ./cmd/gor/
